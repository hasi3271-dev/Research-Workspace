...
const DEFAULT_TODO_DATE = "1970-01-01";
...
function mapPlanner(data, sourceFile, plan) {
  ...
  items.forEach((item, index) => {
    ...
    const dueDate = normalizeDate(pickFirst(item, ["dueDate", "due_date", "due", "date", "deadline"]));
    ...
    addEntity(plan, "todos", sourceKey, {
      row: {
        ...
        priority,
        due_on: dueDate,
        todo_date: todoDateOrDefault(dueDate),
...
function walkPortfolio(node, path, context, plan) {
  ...
  if (looksLikeProject(node, path)) {
    nextContext.projectKey = addProjectFromObject(node, path, context.sourceFile, year, plan, nextContext.importSource);
  } else {
    const projectValue = pickFirst(node, ["project", "projectName", "project_name"]);
    if (projectValue) {
      nextContext.projectKey = addProjectFromValue(projectValue, path, context.sourceFile, year, plan, nextContext.importSource);
    }
  }

  if (looksLikePaper(node, path)) {
    if (!nextContext.projectKey) {
      nextContext.projectKey = addProjectFromObject({
        ...node,
        id: `${pickFirst(node, ["id", "doi", "citationKey", "citation_key", "key"]) || stableHash(node)}-project`,
        status: node.stage || node.status,
        next: node.next || node.nextAction || node.next_action
      }, [...path, "project"], context.sourceFile, year, plan, nextContext.importSource);
    }
    nextContext.figureByLabel = new Map();
  ...
  if (looksLikeFigure(node, path)) {
    ...
    rememberFigureKey(nextContext.figureByLabel, node, nextContext.figureKey);
  }
  ...
  if (looksLikePortfolioItem(node, path, nextContext)) {
    ...
    if (isPaperPortfolioItem(node)) {
      addPaperFromObject({
        id: pickFirst(node, ["paper_id", "paperId", "id", "slug", "key"]),
        title: pickFirst(node, ["title", "name"]),
        year,
        stage: node.status,
        ...
        role: node.role,
        skills: node.skills || node.skill,
        sourcePortfolioItem: node
      }, [...path, "paper"], nextContext, plan);
    }
  }
  ...
  for (const [childKey, value] of orderedChildEntries(node)) {
    ...
    } else if (isReferenceCollection(childKey, value)) {
      asArray(value).forEach((reference, index) => {
        addNoteFromObject(reference, [...childPath, String(index)], nextContext, plan, nextContext.importSource || "portfolio", "paper");
      });
      walkPortfolio(value, childPath, nextContext, plan);
    } else if (isLogCollection(childKey, value)) {
      asArray(value).forEach((log, index) => {
        addNoteFromObject(log, [...childPath, String(index)], nextContext, plan, nextContext.importSource || "portfolio", "research");
      });
...
function mapResearchNotes(data, sourceFile, plan) {
  ...
  if (isPlainObject(data) && (isPlainObject(data.portfolio) || Array.isArray(data.papers))) {
    mapResearchNoteWorkspace(data, sourceFile, plan);
  }
}

function mapResearchNoteWorkspace(data, sourceFile, plan) {
  const context = {
    sourceFile,
    importSource: "research-note",
    year: null,
    projectKey: null,
    paperKey: null,
    figureKey: null,
    figureByLabel: new Map()
  };

  for (const [yearKey, items] of Object.entries(data.portfolio || {})) {
    const year = normalizeYear(yearKey);
    asArray(items).forEach((item, index) => {
      const noteContext = { ...context, year };
      addNoteFromObject(item, ["portfolio", yearKey, String(index)], noteContext, plan, "research-note", "portfolio");
      if (looksLikeProject(item, ["portfolio", yearKey, String(index)])) {
        addProjectFromObject(item, ["portfolio", yearKey, String(index), "project"], sourceFile, year, plan, "research-note");
      }
      if (isPaperPortfolioItem(item)) {
        addPaperFromObject({
          id: pickFirst(item, ["paper_id", "paperId", "id", "slug", "key"]),
          title: pickFirst(item, ["title", "name"]),
          year,
          stage: item.status,
          ...
          sourcePortfolioItem: item
        }, ["portfolio", yearKey, String(index), "paper"], noteContext, plan);
      }
    });
  }

  asArray(data.papers).forEach((paper, paperIndex) => {
    const year = normalizeYear(paper.year);
    const paperContext = {
      ...context,
      year,
      projectKey: addProjectFromObject({
        ...paper,
        id: `${pickFirst(paper, ["id", "doi", "citationKey", "citation_key", "key"]) || stableHash(paper)}-project`,
        status: paper.stage || paper.status
      }, ["papers", String(paperIndex), "project"], sourceFile, year, plan, "research-note"),
      figureByLabel: new Map()
    };
    paperContext.paperKey = addPaperFromObject(paper, ["papers", String(paperIndex)], paperContext, plan);
    addNoteFromObject({
      ...paper,
      title: `${paper.title || "Paper"} - paper workspace`,
      body: paper.summary || paper.abstract || paper.title
    }, ["papers", String(paperIndex), "note"], paperContext, plan, "research-note", "paper");

    asArray(paper.figures).forEach((figure, figureIndex) => {
      const figureKey = addFigureFromObject(figure, ["papers", String(paperIndex), "figures", String(figureIndex)], paperContext, plan);
      rememberFigureKey(paperContext.figureByLabel, figure, figureKey);
      addNoteFromObject({
        ...figure,
        body: [
          figure.claim && `Claim: ${figure.claim}`,
          figure.data && `Data: ${stringifyForBody(figure.data)}`,
          figure.next && `Next: ${figure.next}`,
          figure.reference && `Reference: ${figure.reference}`,
          figure.caption && `Caption: ${figure.caption}`
        ].filter(Boolean).join("\n")
      }, ["papers", String(paperIndex), "figures", String(figureIndex), "note"], { ...paperContext, figureKey }, plan, "research-note", "figure");
    });

    asArray(paper.tasks).forEach((task, taskIndex) => {
      const linkedFigureKey = findLinkedFigureKey(paperContext.figureByLabel, task);
      addNoteFromObject({
        ...task,
        body: [
          task.title,
          task.memo && `Memo: ${task.memo}`,
          task.linked && `Linked: ${task.linked}`,
          task.priority && `Priority: ${task.priority}`,
          task.status && `Status: ${task.status}`,
          task.due && `Due: ${task.due}`
        ].filter(Boolean).join("\n")
      }, ["papers", String(paperIndex), "tasks", String(taskIndex), "note"], { ...paperContext, figureKey: linkedFigureKey }, plan, "research-note", "paper");
    });

    asArray(paper.references).forEach((reference, referenceIndex) => {
      addNoteFromObject(reference, ["papers", String(paperIndex), "references", String(referenceIndex)], paperContext, plan, "research-note", "paper");
    });

    asArray(paper.logs).forEach((log, logIndex) => {
      addNoteFromObject(log, ["papers", String(paperIndex), "logs", String(logIndex)], paperContext, plan, "research-note", "research");
    });
  });
}

function addProjectFromValue(value, path, sourceFile, year, plan, importSource = "portfolio") {
  const raw = isPlainObject(value) ? value : { title: String(value) };
  return addProjectFromObject(raw, [...path, "project"], sourceFile, year, plan, importSource);
}

function addProjectFromObject(project, path, sourceFile, year, plan, importSource = "portfolio") {
  const title = stringifyValue(pickFirst(project, ["title", "name", "project", "projectName", "project_name"]));
  if (!title) return null;

  const sourceKey = makeSourceKey(importSource, "project", pickFirst(project, ["id", "slug", "key"]), project, path);
...
function addPaperFromObject(paper, path, context, plan) {
  ...
  const importSource = context.importSource || "portfolio";
  const sourceKey = makeSourceKey(importSource, "paper", pickFirst(paper, ["id", "doi", "citationKey", "citation_key", "key"]), paper, path);
...
function addFigureFromObject(figure, path, context, plan) {
  ...
  const importSource = context.importSource || "portfolio";
  const sourceKey = makeSourceKey(importSource, "figure", pickFirst(figure, ["id", "key", "label", "figureLabel", "figure_label"]), figure, path);
...
function addPortfolioItemFromObject(item, path, context, plan) {
  ...
  const importSource = context.importSource || "portfolio";
  const sourceKey = makeSourceKey(importSource, "item", pickFirst(item, ["id", "slug", "key"]), item, path);
...
function addExperienceFromObject(experience, path, context, plan) {
  ...
  const importSource = context.importSource || "portfolio";
  const sourceKey = makeSourceKey(importSource, "experience", pickFirst(experience, ["id", "slug", "key"]), experience, path);
...
function addTaskFromObject(task, path, context, plan) {
  ...
  const importSource = context.importSource || "portfolio";
  const sourceKey = makeSourceKey(importSource, "task", pickFirst(task, ["id", "key", "taskId", "todoId"]), task, path);
  const linkedFigureKey = findLinkedFigureKey(context.figureByLabel, task);
  const dueDate = normalizeDate(task.due || task.dueDate || task.deadline || task.date);

  addEntity(plan, "todos", sourceKey, {
    projectSourceKey: context.projectKey,
    paperSourceKey: context.paperKey,
    paperFigureSourceKey: linkedFigureKey || context.figureKey,
    ...
    row: {
      ...
      metadata: compactObject({
        import_source: importSource,
        task_scope: linkedFigureKey || context.figureKey ? "figure" : context.paperKey ? "paper" : "portfolio",
        linked: task.linked,
...
function addNoteFromObject(note, path, context, plan, importSource, noteTypeOverride) {
  ...
  addEntity(plan, "notes", sourceKey, {
    ...
    row: {
      title,
      body,
      note_type: normalizeNoteType(noteTypeOverride || raw.type || raw.category || importSource),
...
function extractNotes(data) {
  ...
  if (pickFirst(data, ["title", "name", "heading", "body", "content", "text", "note", "memo", "summary"])) {
    return [data];
  }
...
function looksLikeProject(obj, path) {
  const key = path[path.length - 1] || "";
  const category = stringifyValue(obj.category) || "";
  return Boolean(
    pickFirst(obj, ["title", "name", "project", "projectName", "project_name"]) &&
    (/projects?/i.test(key) || /프로젝트|project/i.test(category))
  );
}
...
function isFigureCollection(key, value) {
  return Array.isArray(value) && /^(figures|paperFigures|paper_figures)$/i.test(key);
}

function orderedChildEntries(node) {
  return Object.entries(node).sort(([aKey, aValue], [bKey, bValue]) => {
    const aRank = isFigureCollection(aKey, aValue) ? 0 : isTaskCollection(aKey, aValue) ? 1 : 2;
    const bRank = isFigureCollection(bKey, bValue) ? 0 : isTaskCollection(bKey, bValue) ? 1 : 2;
    return aRank - bRank;
  });
}

function isReferenceCollection(key, value) {
  return Array.isArray(value) && /^(references|refs|citations)$/i.test(key);
}

function isLogCollection(key, value) {
  return Array.isArray(value) && /^(logs|history|timeline)$/i.test(key);
}
...
function isPaperPortfolioItem(item) {
  const category = stringifyValue(item.category) || "";
  const title = stringifyValue(item.title) || "";
  return /논문실적|paper|publication|manuscript/i.test(category) || /논문|paper|publication|manuscript/i.test(title);
}

function rememberFigureKey(map, figure, sourceKey) {
  ...
  for (const value of values) {
    map.set(value, sourceKey);
  }
}

function findLinkedFigureKey(map, task) {
...
function normalizeLinkText(value) {
  ...
  if (!text) return null;
  return text.toLowerCase().replace(/\s+/g, "").replace(/^figure/, "fig.");
}
...
function todoDateOrDefault(value) {
  return normalizeDate(value) || DEFAULT_TODO_DATE;
}
...
function normalizePriority(value) {
  ...
  if (["urgent", "highest", "high", "높음", "높은", "긴급", "상"].includes(text)) return 1;
  if (["medium", "normal", "보통", "중간", "중"].includes(text)) return 3;
  if (["low", "lowest", "낮음", "낮은", "하"].includes(text)) return 5;
  return 3;
}
...
function normalizeTodoStatus(value) {
  ...
  if (["in_progress", "doing", "진행_중", "진행중"].includes(status)) return "in_progress";
  if (["blocked", "waiting", "보류", "대기"].includes(status)) return "blocked";
  if (["done", "complete", "completed", "finished", "완료", "끝남"].includes(status)) return "done";
  if (["canceled", "cancelled", "취소"].includes(status)) return "canceled";
  return "open";
}
